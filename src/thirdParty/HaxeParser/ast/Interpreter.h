...
class Interpreter : public ExprVisitor, public StmtVisitor {
    ...
    QMap<QString, QMap<QString,FunctionType>> inheritableNativeClasses;
    ...
    // 将值转换为字符串
    QString stringify(const Dynamic& value) {
...
public:
     ...
     */
    void registerNativeFunc(const QString& name, FunctionType func) {
     ...
     */
    void registerNativeClass(const QString& clsName, const QMap<QString, FunctionType>& clsMethods)
    {
    ...
    Dynamic visitVariableExpr(const Variable& expr) override {
        ...
        try {
            ...
            // 都找不到，抛异常
            throw std::runtime_error(QString("未定义的变量'%1'。").arg(expr.name.lexeme).toStdString());
        }
    }
    ...
    Dynamic visitFunctionStmt(const FunctionStmt& stmt) override {
        ...
        auto func = [declaration = &stmt, closure = environment, this](const std::vector<Dynamic>& args) mutable -> Dynamic {
            ...
            if (args.size() != declaration->params.size()) {
                throw std::runtime_error(QString("函数 '%1' 参数数量不匹配，期望 %2 个，但你传入了 %3 个。")
                    .arg(declaration->name.lexeme)
                    .arg(declaration->params.size())
                    .arg(args.size()).toStdString());
            }
    ...
    Dynamic createBoundMethod(std::shared_ptr<HInstance> instance,
        ...
        return Dynamic([instance, method, definingClass, this](const std::vector<Dynamic>& args) mutable -> Dynamic {
            ...
            if (args.size() != method->params.size()) {
                throw std::runtime_error(QString("方法 '%1' 参数数量不匹配，期望 %2 个，传入 %3 个。")
                    .arg(method->name.lexeme)
                    .arg(method->params.size())
                    .arg(args.size()).toStdString());
            }
        ...
        if (stmt.superclass) {
            try {
                ...
                if (auto* varExpr = dynamic_cast<Variable*>(stmt.superclass.get())) {
                    if (inheritableNativeClasses.contains(varExpr->name.lexeme)) {
    ...
    Dynamic visitGetExpr(const Get& expr) override {
        ...
        if (dynamic_cast<const Super*>(&*expr.object)) {
            ...
            throw std::runtime_error(QString("父类中未找到方法 '%1'。").arg(expr.name.lexeme).toStdString());
    ...
    Dynamic visitNewExpr(const NewExpr& expr) override {
        ...
        if (inheritableNativeClasses.contains(expr.name.lexeme))
        {
            std::vector<Dynamic> args;
            for (const auto& arg : expr.arguments) {
                args.push_back(evaluate(*arg));
        ...
        throw std::runtime_error(QString("未找到类 '%1'。").arg(expr.name.lexeme).toStdString());
    }
