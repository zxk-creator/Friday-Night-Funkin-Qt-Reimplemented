...
class Scanner {
    ...
    // 关键字映射表
    QMap<QString, ETokenType> keywords;
    // 判断是否已扫描到源代码末尾
    bool isAtEnd() { return current >= source.length(); }
    // 消费当前字符并返回，同时向前移动扫描位置
    QChar advance() { return source[current++]; }
    // 查看当前字符但不消费
    QChar peek() {
        if (isAtEnd()) return '\0';
    ...
    void addToken(ETokenType type, QString literal) {
    ...
    bool isAlpha(QChar c) {
        return (c >= 'a' && c <= 'z') ||
               (c >= 'A' && c <= 'Z') ||
               c == '_' ||
               (c.toLatin1() & 0x80);  // UTF-8 多字节字符（中文等）
    }
    // 判断字符是否为字母、数字或下划线
        ...
        switch (c.toLatin1()) {
            ...
            default:
                if (isDigit(c)) {
                    ...
                    qDebug().noquote() << QString("[行 %1] 错误: 非法的字符%2").arg(line).arg(QChar(c));
                }
                break;
        }
    }
    ...
    void string() {
        ...
        if (isAtEnd()) {
            qDebug().noquote() << QString("[行 %1] 错误: 无法中断的字符串。你是否忘记了另一个\"？").arg(line);
            return;
        }
    ...
    void number() {
        ...
        QString value = source.mid(start, current - start);
    ...
    void identifier() {
        ...
        ETokenType type = keywords.value(text, ETokenType::IDENTIFIER);
        addToken(type);
    }

public:
    Scanner(QString source) : source(source) {
